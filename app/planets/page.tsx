  ...
  return (
    <main className="relative min-h-dvh overflow-hidden">
      ...
      <header className="mx-auto flex max-w-6xl items-center justify-between px-6 py-6">
        <Logo />
        <Link href="/profile" className={cn(buttonVariants({ variant: 'ghost' }), 'h-9 rounded-lg')}>
          Profile
        </Link>
      </header>
        ...
        <div className="mt-16 grid gap-8 md:grid-cols-3">
          {planets.map((planet) => (
            <article
              ...
              style={{
                boxShadow: `0 0 0 1px oklch(1 0 0 / 0.06)`,
              }}
            >
              <div
                className="pointer-events-none absolute inset-0 rounded-3xl opacity-0 transition-opacity duration-300 group-hover:opacity-100"
                style={{
                  background: `radial-gradient(circle at 50% 0%, color-mix(in oklab, ${planet.accent} 22%, transparent), transparent 70%)`,
                }}
              />

              <div className="relative transition-transform duration-500 group-hover:scale-105">
                <PlanetOrb accent={planet.accent} accent2={planet.accent2} size={150} />
              </div>

              <h2 className="relative mt-8 text-xl font-semibold">{planet.name}</h2>
              <p className="relative mt-1 text-sm text-muted-foreground">{planet.tagline}</p>
              ...
              <Link
                ...
                className={cn(buttonVariants({ variant: 'default' }), 'group/btn relative mt-6 h-10 w-full rounded-xl text-sm')}
              >
                Enter Planet
