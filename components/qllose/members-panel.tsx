...
export function MembersPanel({ members, typingUsers }: MembersPanelProps) {
  ...
  return (
    <aside className="hidden h-full w-60 flex-col border-l border-border bg-sidebar/50 backdrop-blur-xl lg:flex">
      <div className="border-b border-border px-4 py-4">
        ...
        <p className="text-xs text-muted-foreground">{online.length} online</p>
      </div>
...
function MemberGroup({ label, members }: { label: string; members: Member[] }) {
  return (
    <div className="mb-3">
      <p className="px-2 py-1.5 text-xs font-medium uppercase tracking-wider text-muted-foreground">{label}</p>
      <ul className="flex flex-col gap-0.5">
        {members.map((m) => (
          ...
          >
            <Avatar initials={m.initials} color={m.color} size={32} online={m.online} />
